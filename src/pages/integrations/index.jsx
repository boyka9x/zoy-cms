...
export default function Integration() {
    ...
    const handleConnectShopify = async () => {
        try {
            ...
            setError('');
            if (domain) {
                await shopApi.integration({
                    shopify_domain: domain,
                });

                window.open(
                    `https://${domain}/admin/oauth/redirect_from_cli?client_id=df5796e07bd3899b54f11db327f9d4b3`,
                    '_blank'
                );
            }
        } catch (error) {
            console.log(error);
            toast.error(
                error?.response?.data?.message || 'An error occurred while connecting to Shopify'
            );
        }
    };
            ...
            <Paper elevation={1} sx={{ p: 2, mt: 2, backgroundColor: '#fdfdfd' }}>
                <Stack direction='row' spacing={2} alignItems='center'>
                    ...
                    <Stack spacing={0.5}>
                        <Typography variant='body2' sx={{ fontWeight: 'bold' }} component='span'>
                            Shopify&nbsp;
                            {isCheck?.data?.pixel_id ? (
                                <Chip label='Integrated' color='success'>
                                    {' '}
                                </Chip>
                            ) : (
                                <Chip label='Not integrated'> </Chip>
                            )}
                        </Typography>

                        <Typography variant='caption'>
                            Connect your Shopify store domain to start capturing key events like add
                            to cart, checkout, purchases, and more through WebPixel tracking.
                        </Typography>
                    </Stack>
                </Stack>
                ...
                <Stack direction='row' spacing={2} mt={2}>
                    ...
                    <Button variant='contained' size='small' onClick={handleConnectShopify}>
                        {isCheck?.data?.pixel_id ? 'Connected' : 'Connect'}
                    </Button>
                </Stack>
            </Paper>
        </Container>
